...
How a `--network` run is wired — who talks to whom, and where egress
policy is enforced:

```mermaid
flowchart TB
    subgraph HOST["GPU host"]
        subgraph PROC["gemma4.py process"]
            LOOP["agent loop<br/>native tool-call parser"]
            MODEL["Gemma 4 model<br/>in-process · transformers · bf16 · CUDA<br/>no server, no port"]
            LOOP <-->|"Python calls"| MODEL
        end
        POLICY["policy.yaml<br/>(--policy-file)"]
        CA["MITM CA<br/>(make mitm-ca)"]
    end

    subgraph NETNS["shared network namespace (--network container:proxy)"]
    ...
    WEB(("internet<br/>:80 / :443"))
    DNS(("DNS :53"))

    LOOP -->|"docker exec"| AGENT
    AGENT -->|"all egress"| IPT
    IPT -->|"transparent REDIRECT"| SQUID
    SQUID <-->|"every request:<br/>allow / block / rewrite / adapt"| ICAP
    SQUID -->|"re-encrypted TLS,<br/>upstream verified vs real CA bundle"| WEB
    IPT -->|"allowed out, uninspected"| DNS
...
published. The CA generated by `make mitm-ca` is baked into both images at
build time: the proxy holds the signing key, and the sandbox's stripped trust
store accepts that CA *alone*, so HTTPS that bypasses the proxy fails closed.
Without `--network`, the proxy is never started and the sandbox runs with
`--network none` (no egress at all). DNS is the one uninspected flow — see
the note in the MITM section below.
